...
function id(str) {
  if (str == null) return mongodb.ObjectId();
  return typeof str === 'string' ? mongodb.ObjectId.createFromHexString(str) : str;
}
...
class Manager extends events.EventEmitter {
  constructor(uri, opts, fn) {
    ...
    this._collectionOptions = objectAssign$1({}, DEFAULT_OPTIONS, opts.collectionOptions || {});
  ...
  get(name, options) {
    if ((options || {}).cache === false || !this.collections[name]) {
      ...
      this.collections[name] = new Collection(this, name, objectAssign$1({}, this._collectionOptions || {}, options || {}));
    }
